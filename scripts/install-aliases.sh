...
# Installs the playbook's minimal shell aliases (`pbi`, `pbd`) plus an
# AI_DEV_PLAYBOOK env var into the user's shell rc file. Uses the
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Installs minimal ai-dev-playbook shell aliases.
...
Installed aliases:
  ...
  AI_DEV_PLAYBOOK      Exported env var pointing at the playbook clone
...
render_aliases_file() {
  cat <<ALIASES
# ai-dev-playbook aliases (managed by install-aliases.sh)
#
...
#
# Re-run installer:
#   bash "\${AI_DEV_PLAYBOOK:-\$HOME/ai-dev-playbook}/scripts/install-aliases.sh"

export AI_DEV_PLAYBOOK="\${AI_DEV_PLAYBOOK:-$PLAYBOOK_ROOT}"

# pbi: bootstrap the current project (interactive or pass --tool)
alias pbi='bash "\$AI_DEV_PLAYBOOK/scripts/playbook-init.sh"'

# pbd: diagnose the current project (human-readable)
alias pbd='bash "\$AI_DEV_PLAYBOOK/scripts/playbook-doctor.sh"'
ALIASES
}
...
Workaround — paste the following into your ~/.config/fish/config.fish:
  ...
  set -x AI_DEV_PLAYBOOK "$HOME/ai-dev-playbook"
  alias pbi="bash \"$AI_DEV_PLAYBOOK/scripts/playbook-init.sh\""
  alias pbd="bash \"$AI_DEV_PLAYBOOK/scripts/playbook-doctor.sh\""
...
case "$MODE" in
  ...
  install)
    echo "=== Installing ai-dev-playbook aliases ($shell_kind) ==="
    echo ""
    ...
    echo "  AI_DEV_PLAYBOOK → $PLAYBOOK_ROOT"
    echo ""
