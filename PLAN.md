...
|------|----------|-----------|
| 2026-04-27 | **Adopt bd 1.0.3 surface area in rules and scripts; default-on `bd hooks install` in playbook-init** | Align with beads v1.0.3: document `bd ready --explain`, `--exclude-label`, `bd ping` in recovery, `bd prune` in hygiene; run `bd hooks install` after init unless `--no-hooks`; doctor checks ping + hook presence. Repo remediated with `bd doctor --fix --yes` and gitignore updates. |
