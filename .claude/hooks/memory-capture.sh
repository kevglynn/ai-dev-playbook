...
#          "DECISION: ..." / "FACT: ..." / "PATTERN: ..." / "FAILED_APPROACH: ..."
...
echo "$COMMAND" | grep -qE '(INVESTIGATION:|LEARNED:|DECISION:|FACT:|PATTERN:|FAILED_APPROACH:)' || exit 0
...
for PREFIX in INVESTIGATION LEARNED DECISION FACT PATTERN FAILED_APPROACH; do
...
if [[ "$TYPE" == "failed_approach" ]]; then
  # Must contain a separator indicating reasoning follows
  if ! echo "$CONTENT" | grep -qE ' — | because | since | due to | but '; then
    exit 0  # No reasoning found, skip this entry
  fi
fi
