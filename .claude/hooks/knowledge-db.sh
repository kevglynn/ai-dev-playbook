...
kb_search() {
  ...
  # failed_approach entries get a ranking boost (-5.0) so they surface
  # when the current problem context matches a previously failed approach.
...
ORDER BY bm25(knowledge_fts, -10.0, -5.0, -2.0, -1.0)
       + CASE WHEN k.type = 'failed_approach' THEN -5.0 ELSE 0 END
LIMIT $TOP_N;
SQL
}
