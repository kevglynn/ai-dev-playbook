...
| Script | What it does |
|--------|-------------|
| `playbook-init.sh` | One-command project setup. Copies rules, initializes beads, creates scratchpad, emits `AGENTS.md`, registers for sync. Supports `--tool cursor\|claude\|both` and `--stealth`. Refuses non-TTY invocation without an explicit `--tool` flag. |
| `playbook-doctor.sh` | Validates playbook setup. Checks rules, beads, scratchpad, sync targets, worktrees, global safety net. `--agent` mode emits a `SUMMARY:` line and structured exit codes (`0`=ok, `2`=bootstrap_needed, `3`=rules_drift, `4`=playbook_outdated, `1`=error) so agents can branch on status. |
...
| `sync-rules.sh` | Multi-format rule sync. `--format cursor\|claude\|all`. Supports `--check` (drift), `--local` (in-repo generation), `--dry-run` (preview). `--safe` (back up locally modified files) is the default; `--unsafe` disables. Validates targets, cleans stale files, survives per-target errors. |
